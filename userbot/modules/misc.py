...
async def killdabot(event):
    ...
    await event.edit("`Goodbye *Bentar Gue shutdown dulu gan*....`")
    if BOTLOG:
...
async def killdabot(event):
    await event.edit("`*Mau buka atas apa bawah? :3*`")
    if BOTLOG:
...
async def bot_support(wannahelp):
    ...
    await wannahelp.edit(
        "Join the OpenUserBot Channel: [HERE](https://t.me/userbotindocloud) \
        \nJoin the OpenUserBot Chat: [HERE](https://t.me/userbotindo)")
...
async def repo_is_here(wannasee):
    ...
    await wannasee.edit(
        "Click [here](https://github.com/GudMeong/oub-bot-2) to open GembelUserJmbod's GitHub page."
    )
